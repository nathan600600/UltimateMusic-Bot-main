...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('pause')
        .setDescription('Met en pause la chanson actuelle'),
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            if (player.paused) {
                const embed = new EmbedBuilder().setDescription('❌ La musique est déjà en pause !');
                return interaction.editReply({ embeds: [embed] })
                    .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
            }
            ...
            const embed = new EmbedBuilder().setDescription('⏸️ Musique mise en pause !');
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));

        } catch (error) {
            console.error('Erreur commande pause :', error);
            const embed = new EmbedBuilder().setDescription('❌ Une erreur est survenue lors de la mise en pause de la musique !');
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
        }
    }
