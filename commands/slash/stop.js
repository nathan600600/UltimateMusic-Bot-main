...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('stop')
        .setDescription('Arrête la musique et déconnecte du salon vocal'),
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const embed = new EmbedBuilder().setDescription('🛑 Musique arrêtée et déconnecté du salon vocal !');
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));

        } catch (error) {
            console.error('Erreur commande stop :', error);
            const embed = new EmbedBuilder().setDescription('❌ Une erreur est survenue lors de l\'arrêt de la musique !');
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
        }
    }
